...
class QBWebRTC extends EventEmitter { // extends EventEmitter because we need to recieve events
  ...
  constructor(hypertyURL, bus, configuration) {
    ...
    this._initProStub(bus, hypertyURL)
  }

  _initProStub(bus, hypertyURL) {
    //TODO: temporal fix
    let msg = {
        type: 'init', from: hypertyURL, to: 'sip://test@rethink-project.eu',
        body: {source: hypertyURL, schema: this._objectDescURL}
    };
  ...
  _onNotification(event) {
    ...
    console.info('[QBWebRTC]: Event Received: ', event)
    switch (event.type) {
      case "create":
        ...
        console.info("[QBWebRTC]: [_onNotification] sending event.ack() ");
        let result = event.ack(); // Acknowledge reporter about the Invitation was received
        console.info("[QBWebRTC]: [_onNotification] event.ack() result is:", result);

        setTimeout( () => {
          // Subscribe to Object
          this._syncher.subscribe(this._objectDescURL, event.url).then((objObserver) => {
            console.info("[QBWebRTC]: [_onNotification] objObserver ", objObserver);
  ...
  invite() {
    ...
    return new Promise((resolve, reject) => {
      ...
      navigator.mediaDevices.getUserMedia(this.constraints).then((stream) =>{
          console.log("[QBWebRTC]: localviodeo")
  ...
  acceptCall() {
    ...
    if ( ! offer ) {
      console.log("[QBWebRTC]: offer was't set in the invitation - data: ", data);
      return;
    }
    console.log("[QBWebRTC]: >>>Constraints", this.constraints);
    navigator.mediaDevices.getUserMedia(this.constraints).then( (stream) => {
      this.trigger('localvideo', stream);
      this.mediaStream = stream;
      ...
      this.pc.setRemoteDescription(new RTCSessionDescription(offer), () => {
        ...
        this.connect(this.partner).then( (objReporter) => {
          console.log("[QBWebRTC]: objReporter created successfully: ", objReporter);
          this.objReporter = objReporter;

          this.pc.createAnswer().then( (answer) => {
            this.objReporter.data.connectionDescription = answer;
            this.pc.setLocalDescription(new RTCSessionDescription(answer), () => {
              console.log("[QBWebRTC]: localDescription (answer) successfully set: ", answer);
            });
          });
        });
      });
  ...
  handleObserverObject(dataObjectObserver) {
    ...
    console.info("[QBWebRTC]: handleObserverObject Peer Data:", peerData);
    ...
    if (peerData.hasOwnProperty('iceCandidates')) {
      peerData.iceCandidates.forEach( (ice) => {
        console.log("[QBWebRTC]: handleObserverObject for ice", ice);
        this.processPeerInformation(ice);
      });
    }

    dataObjectObserver.onChange('*', (event) => {
      console.debug('[QBWebRTC]: Observer on change message: ', event);
      ...
      if (event.data[0]) { // [0] this does the trick when ice candidates are trickling ;)
        ...
        console.log('[QBWebRTC]: >>event', event);
  ...
  processPeerInformation(data) {
    console.info("[QBWebRTC]: processPeerInformation: ", JSON.stringify(data));
    //this.createPC();
    if ( ! this.pc ) {
      console.info("[QBWebRTC]: processPeerInformation: no PeerConnection existing --> maybe in disconnecting process. --> ignoring this update");
      return;
    }

    if (data.type === 'offer' || data.type === 'answer') {
      console.info('[QBWebRTC]: Process Connection Description: ', data);
      this.pc.setRemoteDescription(new RTCSessionDescription(data)).then(() => {
        console.log("[QBWebRTC]: remote success")
      })
      .catch((e) => {
        console.log("[QBWebRTC]: remote error: ", e)
      });
    }
  ...
  disconnect() {
    console.log('[QBWebRTC]>>>lets disconnect', this);
    return new Promise( (resolve, reject) => {
      try {
        if (this.objReporter) {
  ...
  switchLocalAudio(newState) {
    console.log('[QBWebRTC] --> setting local audio to: ' + newState);
    try {
  ...
  switchLocalVideo(newState) {
    console.log('[QBWebRTC] --> setting local video to: ' + newState);
    try {
...
export default function activate(hypertyURL, bus, configuration) {
  return {
    name: 'QBWebRTC',
    instance: new QBWebRTC(hypertyURL, bus, configuration)
  };
}
