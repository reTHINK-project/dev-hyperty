...
//Disabled Switch
    let subscribeAuto = '<div class="row"><span class="white-text">' +
            'Auto Subscribe' +
            '<div class="switch white-text">' +
            '<label class="white-text">' +
            'Off' +
            '<input type="checkbox" id="autoSubSwitch">' +
  ...
  $cardPanel.append(subscribeAuto);
...
function notificationHandler(controller, identity) {
  ...
  acceptBtn.on('click', (e) => {
    ...
    getUserMedia(options).then((mediaStream) => {
      ...
      console.error("getUserMedia error :", reason);
    });
  });
...
function processStream(event, user) {
  ...
  if ((event.stream.getVideoTracks().length) > 0) {
    ...
    remoteVideo.id = userSplit[0];
    remoteVideo.autoplay = true;
    remoteVideo.src = URL.createObjectURL(event.stream);
    $('#video-container').append(remoteVideo);

  } else {
    //Stream is an audio only stream
    let audioDiv = document.createElement("div");
    let remoteAudio = document.createElement("audio");
    remoteAudio.autoplay = true;
    remoteAudio.src = URL.createObjectURL(event.stream);
    audioDiv.id = userSplit[0];
    audioDiv.setAttribute("style", "margin-left: 5px; display:inline-block;");
    audioDiv.append(remoteAudio);

    var img = document.createElement('img');
    img.src = 'https://image.freepik.com/icones-gratuites/voix-symbole-de-microphone_318-43222.jpg';
    img.setAttribute("style", "width:450px; height:337px; border-width:2px; border-style:solid; border-color:black;");
    audioDiv.append(img);
    $('#video-container').append(audioDiv);
  }
...
/*
function subscribeToVideoFct() {
controller.subscribeToVideo();
}
*/
...
function showVideo(controller) {
  ...
  if ($('#autoSubSwitch').prop('checked')) {
    console.error("autoSubSwitch checked");
    controller.autoSubscribeActivated = true;
  } else {
    console.error("autoSubSwitch not checked");
    controller.autoSubscribeActivated = false;
  }

  controller.onAddStream((event, user) => {
    console.debug('controller.onAddStream : ', controller, event, user);
    processStream(event, user);
  });

  controller.onParticipant((message) => {
    console.error('controller._onParticipant', message);

    let partList = $('.my-participants-list');
    var userSplit = null;

    if (message.id  === 'newParticipantArrived') {

      userSplit = message.name.split("@");
      $("#" + userSplit[0] + "-ID").remove();
      partList.append('<div id="' + userSplit[0] + '-ID">' + message.name +
       ' - <a class = "black-text" onclick="globalController.subscribeToVideo(' + "'" + message.name + "'" +');"><i>Video Subscribe</i></a><a class = "black-text" onclick="globalController.subscribeToAudio(' + "'" + message.name + "'" +');"><i> - Audio Subscribe</i></a></div>');
      partList.removeClass('hide');

    } else if (message.id === 'existingParticipants') {
      message.data.forEach(function(senderName) {
        userSplit = senderName.split("@");
        $("#" + userSplit[0] + "-ID").remove();
        partList.append('<div id="' + userSplit[0] + '-ID">' + senderName + ' - <a class = "black-text" onclick="globalController.subscribeToVideo(' + "'" + senderName + "'" +');"><i>Video Subscribe</i></a><a class = "black-text" onclick="globalController.subscribeToAudio(' + "'" + senderName + "'" +');"><i> -Audio Subscribe</i></a></div>');
        partList.removeClass('hide');
      });
    } else {
      console.error('message not defined type');
    }
  });

  controller.onCleanStream((userName) => {
    console.error('Clean stream requested for :', userName);
