...
'use strict';Object.defineProperty(exports,"__esModule",{value:true});exports.default=function(dataObject,position,identity){return Object.assign(Object.create(GroupChat),{id:dataObject.url,_dataObject:dataObject,name:dataObject.data.name,startingTime:dataObject.data.startingTime,messages:[],position:position,participants:dataObject.data.participants,identity:identity});};var _GroupChatMessage=require('./GroupChatMessage');var _GroupChatMessage2=_interopRequireDefault(_GroupChatMessage);function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{default:obj};}var GroupChat={sendMessage:function sendMessage(message,distance){var _this=this;return this._dataObject.addChild('chatmessages',{message:message,distance:distance,position:this.position,startingTime:Date.now()}).then(function(child){console.log('message sended',child);_this.messages.push((0,_GroupChatMessage2.default)(child,true,_this.identity));return _this.messages[_this.messages.length-1];});},onMessage:function onMessage(callback){var _this2=this;this._dataObject.onAddChild(function(child){console.log('message received',child);var childData=child.data?child.data:child.value;if(childData.distance&&_this2._distance(_this2._mapPosition(_this2.position.values),_this2._mapPosition(childData.position.values),'K')>childData.distance)return;_this2.messages.push((0,_GroupChatMessage2.default)(child,false,child.identity.userProfile));callback(_this2.messages[_this2.messages.length-1]);});},getContext:function getContext(){return this.messages[this.messages.length-1]?this.messages[this.messages.length-1].text:'';},_mapPosition:function _mapPosition(values){var position={};values.forEach(function(value){return position[value.name]=value.value;});return position;},_distance:function _distance(origin,destination,unit){var radlat1=Math.PI*origin.latitude/180;var radlat2=Math.PI*destination.latitude/180;var theta=origin.longitude-destination.longitude;var radtheta=Math.PI*theta/180;var dist=Math.sin(radlat1)*Math.sin(radlat2)+Math.cos(radlat1)*Math.cos(radlat2)*Math.cos(radtheta);dist=Math.acos(dist);dist=dist*180/Math.PI;dist=dist*60*1.1515;if(unit=="K"){dist=dist*1.609344;}if(unit=="N"){dist=dist*0.8684;}return dist;}};module.exports=exports['default'];

},{"./GroupChatMessage":10}],10:[function(require,module,exports){
"use strict";Object.defineProperty(exports,"__esModule",{value:true});exports.default=function(dataObjectChild,isMe,identity){return Object.assign(Object.create(GroupChatMessage),{_dataObjectChild:dataObjectChild,isMe:isMe,text:dataObjectChild.data?dataObjectChild.data.message:dataObjectChild.value.message,startingTime:dataObjectChild.data?dataObjectChild.data.startingTime:dataObjectChild.value.startingTime,identity:identity});};var GroupChatMessage={elapsedMinutes:function elapsedMinutes(){return Math.round((Date.now()-this.startingTime)%86400000%3600000/60000);}};module.exports=exports['default'];

},{}],11:[function(require,module,exports){
'use strict';Object.defineProperty(exports,"__esModule",{value:true});var NotificationsTriggerObject={_getHyFor:function _getHyFor(identities){var _this=this;return Promise.all(identities.map(function(p){return _this._discoveryService.discoverHypertiesPerUser(p.email,p.domain).then(function(hyperties){return Object.keys(hyperties).map(function(key){return {key:key,descriptor:hyperties[key].descriptor,lastModified:hyperties[key].lastModified};}).filter(function(desc){return desc.descriptor.endsWith('NotificationsObserver');}).sort(function(a,b){return new Date(a.lastModified)<new Date(b.lastModified)?1:-1;}).shift().key;});}));},trigger:function trigger(recipients,notification){var _this2=this;return this._getHyFor(recipients).then(function(hypertyURLs){return _this2._syncher.create(_this2._objectDescURL,hypertyURLs,{}).then(function(reporter){reporter.onSubscription(function(event){return event.accept();});setTimeout(function(){return reporter.addChild('chatmessages',notification).then(function(child){console.log('notification sended',notification);return child;});},2000);});});}};var NotificationsTrigger=function NotificationsTrigger(domain,syncher,discoveryService){return Object.assign({_syncher:syncher,_discoveryService:discoveryService,_objectDescURL:'hyperty-catalogue://'+domain+'/.well-known/dataschemas/Communication'},NotificationsTriggerObject);};exports.default=NotificationsTrigger;module.exports=exports['default'];
